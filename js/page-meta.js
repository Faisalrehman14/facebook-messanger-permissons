...
const PageMeta = (function () {
  ...
  async function subscribe(pageId, pageToken) {
    return GraphAPI.pagePost(pageToken, `/${pageId}/subscribed_apps`, {
  ...
  async function unsubscribe(pageId, pageToken) {
    return new Promise((resolve, reject) => {
      FB.api(`/${pageId}/subscribed_apps`, 'DELETE', { access_token: pageToken }, (res) => {
        if (res && res.success !== false && !res.error) resolve(res);
        else reject(new Error(res?.error?.message || 'Unsubscribe failed'));
      });
    });
  }
